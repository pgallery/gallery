...
class Categories extends Model
{
    ...
    public function albumCount() {
        return Cache::remember('categories.albumCount_' . $this->id . '_cache', Setting::get('cache_ttl'), function(){
            return $this->hasMany('App\Models\Albums')->count();           
        });
    }

    public function albumActiveCount() {
        return Cache::remember('categories.albumActiveCount_' . $this->id . '_cache', Setting::get('cache_ttl'), function(){
            return $this->hasMany('App\Models\Albums')->where('images_id', '!=', '0')->count();           
        });
    }
    
    public function albumCountPublic() {
        return Cache::remember('categories.albumCountPublic_' . $this->id . '_cache', Setting::get('cache_ttl'), function(){
            return $this->hasMany('App\Models\Albums')->where('albums.permission', 'All')->count();           
        });
    }     

    public function albumActiveCountPublic() {
        return Cache::remember('categories.albumActiveCountPublic_' . $this->id . '_cache', Setting::get('cache_ttl'), function(){
    ...
    public function albumCountPrivate() {
        return Cache::remember('categories.albumCountPrivate_' . $this->id . '_cache', Setting::get('cache_ttl'), function(){
            return $this->hasMany('App\Models\Albums')->where('albums.permission', '!=', 'All')->count();           
        });
    }
    
    public function owner() {
        return Cache::remember('categories.owner_' . $this->users_id . '_cache', Setting::get('cache_ttl'), function(){
